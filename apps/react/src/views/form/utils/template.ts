...
export const getFormContent = (formAttribute: TemplateFormAttribute) => {
  ...
  const formAttributeString = Object.keys(attribute).reduce(
    (cur: string, item: string) => {
      if (item === 'colon')
        return `${cur} ${attribute[item] === false ? 'colon={false}' : ''}`;
  ...
  const formItemString = columns.map((column: any) => {
    const { type, key, options, ...formItemAttribute } = column;
    const formItemAttributeString = Object.keys(formItemAttribute).reduce(
      (cur: string, attribute: string) => {
        const value = column[attribute];
        if (attribute === 'valuePropName' && !value?.length) return cur;
        if (['rules'].includes(attribute))
          return `${cur} ${attribute}={${JSON.stringify(value)}}`;
    ...
    <${componentName} ${
      !formItemAttribute.valuePropName?.length &&

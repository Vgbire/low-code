...
import {
  TemplateFormAttribute,
  getFormContent,
  getFormHookName,
} from 'src/views/form/utils/template';
import {
  TemplateTableAttribute,
  getTableColumnsDataSource,
  getTableContent,
} from 'src/views/table/utils/template';
import {
  BodyType,
  ComponentAttribute,
} from '../component/ComponentGlobalControl';
...
export const getBodyFormName = (
  body: BodyType[],
  formAttribute: TemplateFormAttribute
) => {
  if (body.some((item) => item.type === 'form'))
    return getFormHookName(formAttribute);
  return '';
};

export const getBodyTableColumnsDataSource = (
  body: BodyType[],
  tableAttribute: TemplateTableAttribute
) => {
  if (body.some((item) => item.type === 'table'))
    return getTableColumnsDataSource(tableAttribute);
  return '';
};

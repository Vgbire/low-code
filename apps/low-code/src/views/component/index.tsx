...
const Component = () => {
  ...
  // 全局配置属性
  const [componentAttribute, setComponentAttribute] =
    useState<ComponentAttribute>({
      name: 'ComponentName',
  ...
  useEffect(() => {
    setTemplateList(
      JSON.parse(localStorage.getItem(StorageNameMap.component) || '[]')
    );
  }, []);
      ...
      <Col span={8} style={{ padding: '10px 15px 10px' }}>
        {!isForm && !isTable && (
          <ComponentGlobalControl
            ...
            setComponentAttribute={setComponentAttribute}
          />
        )}
